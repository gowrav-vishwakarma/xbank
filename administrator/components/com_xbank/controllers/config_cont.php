...
class config_cont extends CI_Controller{
    function  __construct() {
        ...
        if($u->usertype != "Super Administrator"){
            xRedirect("index.php?option=com_xbank&task=com_xbank.index","You are not Authorised to Configure the system","error");
        }
    }
    ...
    function saveConfig(){
        if($this->input->post("task")=="config_cont.index")
                xRedirect("index.php?option=com_xbank&task=config_cont.index","Back To Configuration Dashboard");
        ...
        foreach($params as $key=>$value)
            $c->setkey($key,$value);
        $c->save();
        xRedirect("index.php?option=com_xbank&task=config_cont.index","Configuration Saved");
    }
