...
class test extends CI_Controller {
    ...
    function runDDS() {
        ...
        foreach ( $accounts as $ac ) {
            ...
            // $ac is actually TRANSACTIONS not account ... variable name taken wrong
            ...
            if ( $acc->agents_id !== null && $acc->agents_id != 0 ) {
                ...
                // Deduct Extra amount submitted after current Date
                $q="SELECT sum(amountCr) extra_amount_submitted from jos_xtransactions t where accounts_id=".$ac->accounts_id . " AND t.created_at > '". getNow( "Y-m-d" ) ."'";
                $extra_amount_submitted = $this->db->query($q)->row()->extra_amount_submitted;

                $tA  = $tA - $extra_amount_submitted;
