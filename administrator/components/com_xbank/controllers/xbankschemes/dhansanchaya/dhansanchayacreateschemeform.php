<?php
$form = $this->form->open("NewDhanSanchayaScheme", 'index.php?option=com_xbank&task=schemes_cont.newscheme')
                ->setColumns(2)
...
// 		->select("Posting Mode","name='PostingMode' class='not-req' not-req-val='-1'",array('Select Intrest Mode'=>'-1','Yearly'=>'Y','Half Yearly' => 'HF','Quarterly'=>'Q','Monthly'=>'M','Weekly'=>'W','Daily'=>'D'))
                ->_()
                ->text("Agent Commissions(in %)", "name='AccountOpenningCommission' class='input req-string tooltip' title='Give account commissions in %. Give comma separated values starting with 0 in RD Scheme. e.g. 0,20,16,9,7' value='0'")
...
//                ->text("Depriciation % After September", "name='DepriciationPercentAfterSep' class='input'")
                if($xc->getKey("number_of_agent_levels")){
                 ...
                 ->submit("Create");
$this->jq->addTab(1, "Dhan Sanchaya Schemes", $this->form->get());
