<?php
 $from =$this->form->open("MoneyBackScheme", 'index.php?option=com_xbank&task=schemes_cont.newscheme')
                ->setColumns(2)
...
//                ->text("Depriciation % After September", "name='DepriciationPercentAfterSep' class='input'")
                ...
                // Field 'Processing Fee in Percent' used here to save money back period
                ->text("Money Back After (in months)","name='ProcessingFeesinPercent' class='input'")
                ...
                if($xc->getKey("number_of_agent_levels")){
                            for($i=1;$i<=$xc->getKey("number_of_agent_levels");$i++)
                                $from = $from->text("Commission String for Level $i","name='AgentCommissionString$i' class='input'");
                        }

                 $from = $form->confirmButton("Confirm", "New Scheme to create", "index.php?option=com_xbank&task=schemes_cont.confirmSchemesCreateForm&format=raw", true)
                ->submit("Create");
$this->jq->addTab(1, "Money Back Schemes", $this->form->get());
