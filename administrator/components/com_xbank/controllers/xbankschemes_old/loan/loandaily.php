...
$CI = & get_instance();
$loanaccounts = $CI->db->query("select a.* from jos_xaccounts a join jos_xpremiums p on a.id = p.accounts_id join jos_xschemes s on a.schemes_id = s.id where s.SchemeType = '" . ACCOUNT_TYPE_LOAN . "' and date(p.DueDate) = '" . $i . "' and a.ActiveStatus=1 and a.branch_id = " . $b->id)->result();
$transactiondate = date("Y-m-d", strtotime(date("Y-m-d", strtotime($i)) . " -1 day"));
foreach ($loanaccounts as $a) {
    $a = new Account($a->id);
