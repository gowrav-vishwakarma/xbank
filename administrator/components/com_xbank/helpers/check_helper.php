...
function myDateAdd($date=null,$days=30){
    if($date==null) $date=getNow('Y-m-d');    
    $date = date("Y-m-d", strtotime(date("Y-m-d", strtotime($date)) . " +$days DAY"));    
    return $date;
}
...
function convert_digit_to_words($no)  
    {
        $r_p= explode(".", $no);
        echo no_to_words($r_p[0]) . " rupees";
        if(count($r_p)>1){
            if(strlen($r_p[1])==1) $r_p[1].="0";
            if(strlen($r_p[1])>2) $r_p[1] = substr($r_p[1], 0,2);
            if($r_p[1] !="00")  echo " and " .no_to_words($r_p[1]) . " paise";
        } 
    }
