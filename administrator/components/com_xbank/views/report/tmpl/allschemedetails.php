...
elseif($st->SchemeType == 'SavingAndCurrent'){
    $ft=new Transaction();
    $ft->where('accounts_id',$acc->id);
    // $ft->where('transaction_type_id',4); // ammount deposit in saving account
    $ft->where('amountCr <>','0');
    $ft->order_by('id','asc');
    $ft->limit(1);
    $ft->get();
    // echo $ft->check_last_query();
    $total += $ft->amountCr;
