...
<tr>
<td><h1>xCIDeveloper User Guide Version 0.6 </h1></td>
...
<ul class="minitoc">
	...
	<div class="guidelineDetails">
            ...
            <p><code>$this->form->open("formname", "index.php?option=com_employee&controller=con_enquiry&task=con_enquiry.departmentsave")</code> </p>
			The above code will create a form with id 'formname' and action='index.php?option=com_employee&controller=con_enquiry&task=con_enquiry.departmentsave'.
	</div>
...
<p class="important">The Form Library supports at the most 2 columns in a form.</p>
  </div>
...
<code>->lookupDB("Search", "name='search'", "index.php?option=com_xinstitute&task=confirmstudent.searchStudent&format=raw",<br>array("id", "name", "mobile_no", "father_name"), "id")<br />
...
<code>->confirmButton("Confirm", "Are u sure?", "index.php?option=com_xinstitute&task=staff.confirmEditStaff&id=$id&format=raw");</code>		</div>
...
function index() {
	...
	$this->form->open('One','index.php?option=com_sample&task=mod_controller.myfunction')
	->setColumns(2)
	...
	->lookupDB("LookupDB","name='branch' class='input req-string'",
		"index.php?option=com_xinstitute&task=batchdetail.addstudentbatch&format=raw",
		 array("id", "name"), "id")
	->_("mylabel")
	->confirmButton("Confirm", "Confirm Box", "index.php?option=com_sample&task=mod_controller.confirmFunction", true)
	->submit('Submit');
